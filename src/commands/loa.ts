...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('username').setDescription('The Roblox username to place on LOA').setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName('end_date')
        .setDescription('When the LOA ends (MM/DD/YYYY, YYYY-MM-DD, or "2 weeks")')
        .setRequired(true)
    )
    .addStringOption(option =>
      option.setName('notes').setDescription('Additional notes for this LOA').setRequired(false)
    ),
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    // Only supervisors can set LOA for Roblox users
    if (!hasSupervisorPermission(memberRoleIds, guildId)) {
      return interaction.editReply(
        `You do not have permission to use this command. Only supervisors can place members on ${terms.loa}.`
      );
    }
    ...
    addAuditLog(
      ...
      'LOA_CREATE',
      targetUserId,
      `Placed ${username} on ${terms.loa} until ${endDate.toLocaleDateString()}`
    );

    return interaction.editReply(
      `Successfully placed **${username}** on ${terms.loa} until <t:${Math.floor(endDate.getTime() / 1000)}:D>.`
    );
  }
};
