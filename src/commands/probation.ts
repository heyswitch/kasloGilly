...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('probation')
    .setDescription('Place a member on Probation')
    .addStringOption(option =>
      option.setName('username').setDescription('The Roblox username to place on probation').setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName('end_date')
        .setDescription('When the probation ends (MM/DD/YYYY, YYYY-MM-DD, or "2 weeks")')
        .setRequired(true)
    )
    .addStringOption(option =>
      option.setName('notes').setDescription('Additional notes for this probation').setRequired(false)
    ),
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    // Only supervisors can set probation for Roblox users
    if (!hasSupervisorPermission(memberRoleIds, guildId)) {
      return interaction.editReply(
        `You do not have permission to use this command. Only supervisors can place members on ${terms.probation}.`
      );
    }
    ...
    addAuditLog(
      ...
      'PROBATION_CREATE',
      targetUserId,
      `Placed ${username} on ${terms.probation} until ${endDate.toLocaleDateString()}`
    );

    return interaction.editReply(
      `Successfully placed **${username}** on ${terms.probation} until <t:${Math.floor(endDate.getTime() / 1000)}:D>.`
    );
  }
};
