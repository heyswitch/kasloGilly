...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('username').setDescription('The Roblox username to suspend').setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName('end_date')
        .setDescription('When the suspension ends (MM/DD/YYYY, YYYY-MM-DD, or "2 weeks")')
        .setRequired(true)
    )
    .addStringOption(option =>
      option.setName('notes').setDescription('Additional notes for this suspension').setRequired(false)
    ),
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    // Only supervisors can suspend Roblox users
    if (!hasSupervisorPermission(memberRoleIds, guildId)) {
      return interaction.editReply(
        `You do not have permission to use this command. Only supervisors can suspend members.`
      );
    }
    ...
    addAuditLog(
      ...
      'SUSPENSION_CREATE',
      targetUserId,
      `Suspended ${username} until ${endDate.toLocaleDateString()}`
    );

    return interaction.editReply(
      `Successfully suspended **${username}** until <t:${Math.floor(endDate.getTime() / 1000)}:D>.`
    );
  }
};
